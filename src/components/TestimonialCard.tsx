...
export function TestimonialCard({ testimonial, className }: TestimonialCardProps) {
  ...
  const quoteText = typeof testimonial.quote === "string" ? testimonial.quote : tl(testimonial.quote);
  const name = testimonial.name || "Guest";
  const country = testimonial.country || "International";
  const trip = testimonial.trip || "Bespoke Journey";
  const image = testimonial.image || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=400&q=80";
  ...
  return (
    ...
    >
      ...
      <div className="pt-4 border-t border-slate-100 flex items-center gap-3">
        ...
        <div>
          <div className="text-sm font-bold text-[#081A33]">{name}</div>
          <div className="text-xs text-slate-400 flex items-center gap-1.5 font-normal">
            <span>{country}</span>
            <span>•</span>
            <span className="text-[#C8A45D] font-medium">{trip}</span>
          </div>
        </div>
      </div>
    </div>
